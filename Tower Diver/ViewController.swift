...
class ViewController: UIViewController {
    ...
    @IBAction func DisplayHP(){
        ...
        var HealthString: String = Name + "\nCurrent HP: " + DoubleString(Input: CurrentHP) + "\nMax HP: " + DoubleString(Input: MaxHP) + "\nClass: " + ClassString
        if(isBleeding){
        ...
        if(ReviveCounter != 999){
            HealthString = HealthString + "\n" + String(ReviveCounter) + " Revive Counters"
    ...
    func ReadStats(){
        ...
        hasShoes = PullBool(Stat: "hasShoes")
        ...
        inHell = PullBool(Stat: "inHell")
        
        isBleeding = PullBool(Stat: "isBleeding")
        isConfused = PullBool(Stat: "isConfused")
        ConfusionCounter = PullInt(Buff: "confusionCounter")
        BleedingCounter = PullInt(Buff: "bleedingCounter")
        BattlePredict = PullBool(Stat: "BattlePredict")
        
        Theme = PullString(Stat: "MonsterTheme")
        
        ReviveCounter = PullInt(Buff: "ReviveCounter")
    }
    ...
    func Dead(){
        ...
        bottomEvent("Revive", #selector(Revive))
    }
    ...
    func DetectJailbreak() -> Bool{
        ...
        if FileManager.default.fileExists(atPath: "/Applications/Cydia.app")
            ...
            || UIApplication.shared.canOpenURL(URL(string:"zebra://package/com.example.package")!){
            return true
        }
        ...
        if(SKPaymentQueue.canMakePayments()){
            SwiftyStoreKit.purchaseProduct(ProductID, quantity: 1, atomically: true) { result in
                ...
                case .error(let error):
                    ...
                    case .privacyAcknowledgementRequired: self.Alert("privacyAcknowledgementRequired")
                    case .unauthorizedRequestData: self.Alert("unauthorizedRequestData")
                    case .invalidOfferIdentifier: self.Alert("invalidOfferIdentifier")
                    case .invalidSignature: self.Alert("invalidSignature")
                    case .missingOfferParams: self.Alert("missingOfferParams")
                    case .invalidOfferPrice: self.Alert("invalidOfferPrice")
                    @unknown default: self.Alert("Unknown error. Please contact support")
                    }
                }
            }
        }else{
    ...
    func LogCharacter(){
        ...
        var CLog = Name + "::" + String(Class) + "::"
        CLog = CLog + String(Floor) + "::" + String(didEscape)
        let Total: Int = defaults.integer(forKey: "TotalC") + 1
        defaults.set(CLog, forKey: "CLog_" + String(Total))
        defaults.set(Total, forKey: "TotalC")
    ...
    func SaveStats(){
        ...
        Save(Input: hasShoes, Key: "hasShoes")
        ...
        Save(Input: BleedingCounter, Key: "bleedingCounter")
        Save(Input: isConfused, Key: "isConfused")
        Save(Input: ConfusionCounter, Key: "confusionCounter")
        Save(Input: ReviveCounter, Key: "ReviveCounter")
        Save(Input: inHell, Key: "inHell")
    }
    ...
    func GenEvent(){
        ...
        if(Result >= 0 && Result <= CurseChance ){
            ...
            NSLog("Find Merchant")
            FindMerchant()
        }else if(Result > MerchantChance && Result <= ElevatorChance && Floor < 245){
            NSLog("Find Elevator")
            FindElevator()
    ...
    func FinalFloor(){
        GetAchieve("Master Adventurer")
        if(Class == 5){
            GetAchieve("Toast Master")
        }
   ...
   @IBAction func CastleEvent2_b(){
    ...
    @IBAction func HomeEvent7_a(){
        GetAchieve("Flashbacks From A Better Time")
    ...
    @IBAction func MarketEvent4_a(){
        ...
        GetAchieve("How Assassins Are Born")
    ...
    func GenProduct() -> [String]{
        ...
        if(Type == "Mystery Sack" || Type == "Armor"){
            Total = 1
        }
        let Cost: Double = Double(arc4random_uniform(UInt32(800 * Floor * DecensionAdjustment)) + UInt32(Floor * 50))
    ...
    func FindMerchant(){
        ...
        }else if(CurrentProduct[0] == "Armor"){
            AdventureLog.text = "You come across a merchant offering a nice set of " + GenArmorType() + " for " + CurrentProduct[2] + "G."
        }else{
    ...
    @IBAction func BuyProduct(){
        ...
        if(Gold < Cost){
            ...
            case "Mystery Sack":
                GiveMysterySack()
            case "Armor":
                GiveArmor(SkipText: false, BlackSmith: false, UseNext: true)
            default:
                GivePotion(SkipText: true, UseContainer: false, Total: Double(CurrentProduct[1]) ?? 1)
                SetLabels()
                AdventureLog.text = AdventureLog.text + "\n" + "Come again!"
    ...
    func GiveArmor(SkipText: Bool = false, BlackSmith: Bool = false, UseNext: Bool = false){
            ...
            if(!SkipText && BlackSmith){
                MainImage.image = UIImage(named: "Blacksmith.png")
                AdventureLog.text = AdventureLog.text + "\nThe blacksmith upgraded your " + GenArmorType() + "!\nHealth increased by: " + DoubleString(Input: PowerLevel)
            }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if(!defaults.bool(forKey: "DoTutorial")){
            if(defaults.bool(forKey: "hasCharacter")){
                ...
                if(defaults.bool(forKey: "ShouldAchieve")){
                    defaults.set(false, forKey: "ShouldAchieve")
                    DispatchQueue.main.async(execute: {
                        self.performSegue(withIdentifier: "AchievementSegue", sender: nil)
                    })
                }
    ...
    func GetAchieve(_ Achievement: String){
        if(!defaults.bool(forKey: "A_" + Achievement)){
            let AT = AchieveTable();
    ...
    func CheckForAchieves(){
        ...
        if(Floor >= 100){
            GetAchieve("Skilled Adventurer")
        }
        
        if(Floor >= 200){
            GetAchieve("Determined Adventurer")
        }
        ...
        if(Gold >= 100000){
            GetAchieve("Accountant")
        }
        
        if(Gold >= 1000000){
            GetAchieve("Master Of Coin")
        }
        
        if(Power >= 9001){
            GetAchieve("It's Over 9000!")
        }
        
        if(Power >= 100000){
            GetAchieve("Strongman")
        }
        
        if(Power >= 1000000){
            GetAchieve("True Power")
        }
        
        if(Power == MaxValue && Gold == MaxValue && MaxHP == MaxValue){
            GetAchieve("I'm God Tier Bro")
        }
    ...
    @IBAction func FindTreasure(){
        ...
        }else if(a >= 101 && a < 110 && !hasESP){
                AdventureLog.text = "The treasure chest engulfs you in light! You now feel smarter!"
                hasESP = true
                SaveStats()
                Next()
        }else if(a >= 110 && !hasESP){
            AdventureLog.text = "The treasure chest contained a pair of winged shoes! You should be able to runaway from anything now!"
            hasShoes = true
            SaveStats()
            Next()
        }else{
    ...
    func Battle(isMimic: Bool = false, MimicName: String = "Mimic"){
        ...
        if (!isMimic && !SquirrelMark)
        {
            ...
            else if(Gen > EnemyChance * 2 && Gen <= ((EnemyChance * 2) + EnhancedChance) && Floor > 5){
                EnhancedEnemy()
            }else if(Gen > ((EnemyChance * 2) + EnhancedChance) && Floor > 10){
                Boss()
            }
            else{
                NormalEnemy()
    ...
    func GetMonster(Type: Int = 1, Overload: String = "NA") -> [String]{
        ...
        var Adjustment: Double = Double(Floor / 25)
        if(Adjustment < 1){
            Adjustment = 0.5
        }
        ...
        let TempPower: String = String((Double(TempArray[1])! * Double(Type * AfterDeath * Floor * 2 * DecensionAdjustment) * Adjustment) / EasyMode)
    ...
    @IBAction func FightEnemy(){
        ...
        if(CurrentHP < 1){
            ...
            let LootedGold: Double = Double(arc4random_uniform(UInt32(Floor * 200)) + UInt32(Floor * 25))
            ...
            if(CurrentMonster[0] == "Succubus" && Theme == "Stick_"){
                GetAchieve("More Than Just A Pretty Face")
            }
    ...
    @IBAction func RunAway_FullLoss(){
        ...
        if(!hasShoes){
            ...
            AdventureLog.text = "You run away losing nothing!"
        }
        SetLabels()
        Next()
    }
    ...
    func GenMiscEvent(){
        let Gen: Int = Int(arc4random_uniform(130))
        if(Gen >= 0 && Gen < 25){
            FindBadMerchant()
            ...
            ConfusionCurse()
        }else if(Gen >= 105 && Gen < 109){
            EncounterGilgamesh()
        }else if(Gen >= 110 && Gen < 120){
            FindBlackSmith()
        }else{
            FindDeadBody()
        }
    ...
    func FindBlackSmith(){
        MainImage.image = UIImage(named: Theme + "Blacksmith.png")
        let Cost: Double = Double(arc4random_uniform(UInt32(800 * Floor * DecensionAdjustment)) + 100)
        CurrentProduct = ["Armor", "1", String(Cost)]
        AdventureLog.text = "You come across a large burly man working at an anvil. He offers to upgrade your armor, for a small price...\n\nUpgrade your armor for " + DoubleString(Input: Cost) + " Gold?"
        topEvent("Upgrade Armor", #selector(DoBlackSmith))
        bottomEvent("Continue", #selector(GenerateEventButton))
    }
    
    @IBAction func DoBlackSmith(){
        if(Gold < Double(CurrentProduct[2]) ?? 420){
            AdventureLog.text = "You don't have enough Gold to pay for his services!"
        }else{
            Gold -= Double(CurrentProduct[2]) ?? 420
            GiveArmor(SkipText: false, BlackSmith: true)
        }
        Next()
    }
    ...
    @IBAction func FightGilgamesh(){
        ...
        if(CurrentHP < 1){
            ...
            GetAchieve("The Greatest Knight")
            Next()
        }
    }
    ...
    @IBAction func DoSquirrelBad(){
        GetAchieve("The Worst Kind Of Person")
    ...
    func FindCurseTrap()
{
    ...
    {
        var EPower: Double = 666 * 20 * Double(AfterDeath * Floor * DecensionAdjustment)
        if(EPower > MaxValue || EPower <= OverFlowed){
            EPower = MaxValue
        }
        CurrentMonster = ["Death", String(EPower), "3"]
        ...
        isStunned = false
        trapSet = false
        DeathHealth = Double(CurrentMonster[1])! * 66
        if(DeathHealth > MaxValue){
            DeathHealth = MaxValue
        }
    ...
    @IBAction func Debug(){
        ...
        alert.addAction(UIAlertAction(title: "Clear Achievements", style: .default, handler: { action in
    ...
    func ClearAchievements(){
        ...
        let Names: [String] = AT.Names
        for item in Names{
            defaults.set(false, forKey: "A_" + item)
        }
    }
    ...
    func EventDebug(){
        ...
        alert.addAction(UIAlertAction(title: "Find Merchant", style: UIAlertAction.Style.default, handler: { action in
            self.FindMerchant()
        }))
        alert.addAction(UIAlertAction(title: "Find Blacksmith", style: UIAlertAction.Style.default, handler: { action in
            self.FindBlackSmith()
        }))
    ...
    func StatDebug(){
        ...
        alert.addAction(UIAlertAction(title: "Change Shoes Status", style: UIAlertAction.Style.default, handler: { action in
            self.hasShoes = !self.hasShoes
            self.SaveStats()
        }))
    ...
    public class LoadingOverlay{
        ...
        public func showOverlay(view: UIView) {
            ...
            imageView.center = CGPoint(x: overlayView.bounds.width / 2, y: overlayView.bounds.height / 2 - 50)
            
            label.frame = CGRect(x: 0, y: 0, width: 400, height: 100)
            label.text = "Loading The Tower"
            ...
            activityIndicator.center = CGPoint(x: overlayView.bounds.width / 2, y: overlayView.bounds.height / 2 + 200)
    ...
    func Tutorial(){
        LoadingOverlay.shared.hideOverlayView()
        defaults.set(false, forKey: "DoTutorial")
        CurrentHP = 100
        MaxHP = 100
        Power = 100
        Gold = 100
        ...
        MainImage.image = UIImage(named: "QuestionMark_.png")
        AdventureLog.text = "Welcome to Tower Diver!\nIn this tutorial you will learn the basics to being an adventurer!\nIf at anytime you would like to stop the tutorial just press the menu button in the bottom right hand corner of your screen and press 'Restart'."
        AdventureLog.setContentOffset(.zero, animated: true)
        topEvent("Let's Get Started!", #selector(Tutorial1))
        bottomEvent("Let's Get Started!", #selector(Tutorial1))
    }
    
    @IBAction func Tutorial1(){
        MainImage.image = UIImage(named: "Tutorial1.png")
        AdventureLog.text = "Let's start with your character.\nDuring your character creation you will be able to select one of three starting classes\nWarrior, Ranger, and Mage.\nEach class has it's own unique abilities and starting stats.\n After selecting your class you will be able to adjust your starting stats. Decrease one stat to raise another!\nYour characters stats are\nHP, or hit points, represent how much damage you can take before dying. Increase it by killing enemies or upgrading your armor.\nPower, represents how much damage you can deal to enemies, the higher your power the less damage you will end up taking from fighting.\nGold, as the name implies its Gold, gold is used to buy items and armor upgrades, you will lose Gold and Potions when you run away from enemies, so be careful!\nAnd Potions. Each potion will restore some of your lost HP, equal to one tenth of your maximum health."
        AdventureLog.setContentOffset(.zero, animated: true)
        topEvent("Battles", #selector(Tutorial2))
        bottomEvent("Battles", #selector(Tutorial2))
    }
    
    @IBAction func Tutorial2(){
        MainImage.image = UIImage(named: "Slime.png")
        AdventureLog.text = "The way battles work in Tower Diver is a bit different than a normal RPG. There are no turns, no defense ratings, no speed. They are simple. You can either choose to fight the enemy or run away. Running away will make you lose Gold and Potions, the stronger the enemy the more you will lose. When you choose to fight, your Power and the Enemy Power are calculated to determine how much HP you as the player will lose. Now you may be thinking, 'I always lose HP? That's a bit strange isn't it?' and you would be right! It is strange. But it adds chalenge to the game, so that essentially, no matter how strong you become you will always have a risk of dying. Obviously the stronger the monster is compared to you, the more HP you will lose, and the stronger you are, you will lose less HP. For a full in-depth guide on how damage is calculated check out our wiki for all the fun maths.\nOne thing you will want to remember, if a monster is significantly stronger than you, there's a good chance that it will kill you without even trying. So try not to advance through the Tower too quickly so you can get stronger!"
        AdventureLog.setContentOffset(.zero, animated: true)
        topEvent("Advancing Through The Tower", #selector(Tutorial3))
        bottomEvent("Advancing Through The Tower", #selector(Tutorial3))
    }
    
    @IBAction func Tutorial3(){
        MainImage.image = UIImage(named: "TitleScreen.png")
        AdventureLog.text = "The Tower is a vast and dangerous place. Each floor is more dangerous than the last.\nOn each floor you will go through Rooms, and on each room you will encounter a new event, whether that be a monster, item, or frienly squirrel, there will always be something waiting for you. Once you go through all the rooms on a floor you will advance to the next floor. You may also find Elevators that will raise you upwards of 5 floors, or you may find a pitfall trap that will drop you floors! As you advance through the floors you will encounter stronger enemies, but you will also find greator weapons and armor!"
        AdventureLog.setContentOffset(.zero, animated: true)
        topEvent("Weapons And Armor", #selector(Tutorial4))
        bottomEvent("Weapons And Armor", #selector(Tutorial4))
    }
    
    @IBAction func Tutorial4(){
        MainImage.image = UIImage(named: "Weapons.png")
        AdventureLog.text = "During your Tower dive, you will find various weapons and armor. But don't worry about being selective about taking them. In the Tower, you can carry as many weapons as you want!!! Many adventurers have asked how that's even possible, but we don't know, we just let the Tower do it's thing in peace.\n\nEach weapon you find will increase your power, the higher the current floor you're on, the stronger it will be. Same goes for finding armor or getting an armor upgrade. Be careful though, some of the weapons you may find are cursed! And can cause some problems for you in the future. But it's worth the Power upgrade right?"
        AdventureLog.setContentOffset(.zero, animated: true)
        topEvent("Curses", #selector(Tutorial5))
        bottomEvent("Curses", #selector(Tutorial5))
    }
    
    @IBAction func Tutorial5(){
        MainImage.image = UIImage(named: "Curse.png")
        AdventureLog.text = "Getting cursed sucks. But it's not the end of the world. Getting cursed in the Tower can make you find more Monsters, traps, or even make you find more items! No matter how much you get cursed though you will still be able to encounter other events. If you ever get too many curses, try and find a Hot Spring, taking a bath in one will remove all effects of nasty Curses."
        AdventureLog.setContentOffset(.zero, animated: true)
        topEvent("Continue", #selector(Tutorial6))
        bottomEvent("Continue", #selector(Tutorial6))
    }
    
    @IBAction func Tutorial6(){
        MainImage.image = UIImage(named: "QuestionMark_.png")
        AdventureLog.text = "And that about sums up the basics of Tower Diver! There's still a lot more to encounter than what was mentioned here, but we don't want to spoil too much of it for you! If you want to read up more about how some things work check out our Wiki page, which you can access from the menu button on the bottom right corner of the screen! Thank you for playing Tower Diver!"
